...
/**
 *this example alone will not record any video , it will just demonstrate to you the webcam interface 
 *to record the video on file you must add this line on the main function :
 * Thread t2=new Thread(new Encoder("file name"));t2.start();
 * 
 * by karima AND yassine
 * 
 * @author karima ichou
 */
public class WebCamAppLauncher extends Application {
	...
	public void start(Stage primaryStage) {
		...
		primaryStage.setTitle("record chapter's lecture");
		root = new BorderPane();
		...
		webCamPane.setStyle("-fx-background-color:  #8db9c9;");
	...
	private void createCameraControls() {
		...
		btnCameraDispose.setText("finish record");
	...
	protected void disposeWebCamCamera() {
		...
		alert.setTitle("recordinf Information");
		alert.setHeaderText(null);
		alert.setContentText("video recorder successfully!");
		alert.showAndWait();	
		stage.close();
	}
